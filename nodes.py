...
class MiniMaxH3MotionContext:
    ...
    def apply(self, conditioning, vae, latent, context_frames, context_length,
        ...
        # FL2VA/Ref2VA keyframe compatibility: a stock first_frame/last_frame
        # anchor from the upstream conditioning node must not be replaced by
        # our own pinned run -- append instead of assigning minimax_keyframes
        # in `values` above. When an audio-continuation ref is also being
        # added, tag every incoming stock anchor with an explicit MC_KEY
        # first (see _tag_stock_keyframes_for_ref_offset), so its position
        # picks up the same ref-cursor offset ours does.
